    ...
    <main className="h-[100dvh] w-full bg-figma-secondary flex justify-center overflow-hidden">
      <div className="w-full max-w-[392px] bg-figma-accent h-[100dvh] flex flex-col relative overflow-hidden shadow-2xl">
        ...
        <div className="relative w-full flex-1 min-h-0 bg-figma-surface overflow-clip max-h-[55dvh]">
          ...
          {/* Capturing overlay (long-exposure process indicator) */}
          {capturing && !flash && exposureMs > 0 && (
            <div className="absolute inset-0 z-40 bg-black/50 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
              <div className="w-12 h-12 border-4 border-[#04d9d9]/30 border-t-[#04d9d9] rounded-full animate-spin" />
              <p className="text-white text-2xl font-bold animate-pulse tracking-wide">Capturando…</p>
            </div>
          )}
          ...
          {/* AR Mode Badge (toggles AR) */}
          <button
            onClick={() => setArActive((v) => !v)}
            className="absolute top-6 left-1/2 -translate-x-1/2 py-1 px-3 bg-figma-surface-2 rounded-[39311300px] z-20 active:scale-95 transition-transform"
          >
